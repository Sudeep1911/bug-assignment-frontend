...
export default function KanbanBoard({
  ...
  const getStageColor = (stageName: string) => {
    ...
    return "bg-slate-600/30 text-slate-200 border-slate-600"; // Default color
  };
  ...
  const handleDrop = (e: React.DragEvent, stageId: string) => {
    e.preventDefault();
    if (draggedBug) {
      // Let the parent component handle the state update and API call
  ...
  const getBugsByStage = (stageId: string) => {
    return bugs.filter(bug => bug.status  === stageId);
  };
    ...
    {stages.map((stage) => (
      ...
      >
        ...
        <div className={`p-4 rounded-t-3xl border-b border-white/20 ${getStageColor(stage.name)}`}>
          ...
          <p className="text-sm">
            {getBugsByStage(stage._id).length} tasks
          </p>
        </div>
                ...
                >
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-slate-200">
                      {getAssigneeName(bug.assignedTo || "")}
                    </span>
