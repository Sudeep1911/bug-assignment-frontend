...
export default function DashboardPage() {
  ...
  const handleSubmitItem = async (data: SubmitItemData) => {
    ...
    try {
      const todoStageId = selectedProject?.kanbanStages?.find((stage: any) => stage.name.toLowerCase() === 'todo')?._id;
  ...
  const handleItemStatusUpdate = async (itemId: string, newStatus: string) => {
    ...
    try {
      ...
      if (response && response.data) {
        ...
        setTasks((prevTasks) => prevTasks.map((task) => (task._id === updatedItem._id ? updatedItem : task)));
        setNotification({
          type: 'success',
  ...
  const handleUpdateItem = async (data: SubmitItemData) => {
    ...
    try {
      ...
      setTasks((prevTasks) => prevTasks.map((task) => (task._id === editedBug._id ? updatedItem : task)));
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-neutral-900 to-slate-900 text-white relative overflow-hidden">
      ...
      <div className="backdrop-blur-lg bg-white/5 border-b border-white/20 px-6 py-4 sticky top-0 z-20">
        <div className="flex items-center justify-between relative">
          ...
          {/* Right section: Create New and User Profile */}
          <div className="absolute right-6 flex items-center space-x-4">
            <div className="relative group">
              <button
                onClick={() => setIsSubmitPopupOpen(true)}
                className="flex items-center justify-center w-10 h-10 bg-gradient-to-r from-purple-500 to-cyan-500 text-white rounded-full transition-all duration-300 shadow-lg"
              >
                <Plus className="w-5 h-5" />
              </button>
              <div className="absolute w-auto left-1/2 transform -translate-x-1/2 top-12 px-3 py-1 bg-gray-800 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
                Create New Task
              </div>
            </div>
